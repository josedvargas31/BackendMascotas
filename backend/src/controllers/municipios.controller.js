...
export const registrarMunicipio = async (req, res) => {
  try {
    const { nombre, fk_id_departamento } = req.body;
    const [result] = await pool.query("INSERT INTO municipios (nombre, fk_id_departamento) VALUES (?, ?)", [nombre, fk_id_departamento]);
...
export const actualizarMunicipio = async (req, res) => {
  try {
    ...
    const { nombre, fk_id_departamento } = req.body;
    const [result] = await pool.query("UPDATE municipios SET nombre=?, fk_id_departamento=? WHERE id_municipio=?", [nombre, fk_id_departamento, id_municipio]);
    if (result.affectedRows > 0) {
      res.status(200).json({
        status: 200,
...
export const eliminarMunicipio = async (req, res) => {
  try {
    const { id_municipio } = req.params;
    const [result] = await pool.query("DELETE FROM municipios WHERE id_municipio=?", [id_municipio]);
    if (result.affectedRows > 0) {
      res.status(200).json({
        status: 200,
...
export const buscarMunicipio = async (req, res) => {
  try {
    const { id_municipio } = req.params;
    const [result] = await pool.query("SELECT * FROM municipios WHERE id_municipio=?", [id_municipio]);
    if (result.length > 0) {
      res.status(200).json({
        status: 200,
